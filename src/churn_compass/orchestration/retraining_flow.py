...
def evaluate_retraining_criteria(
    ...
    if data_drift.get("drift_detected"):
        ...
        if drift_share >= drift_threshold:
            should_retrain = True
            reasons.append(
                f"Significant data drift detected: {drift_share:.1%} of features"
            )
    ...
    if performance_drift and performance_drift.get("performance_degraded"):
        ...
        if pr_auc_drop >= performance_drop_threshold:
            should_retrain = True
            reasons.append(
                f"Performance degradation: PR-AUC dropped by {pr_auc_drop:.3f}"
            )
    ...
    decision = {
        ...
        "reasons": reasons,
        "drift_share": data_drift.get("drift_share", 0),
        "performance_drop": performance_drift.get("metric_drops", {}).get(
            "pr_auc_drop", 0
        )
...
def check_manual_approval(decision: Dict, require_approval: bool = True) -> bool:
    """
    ...
    logger.info(
        ...
        extra={"require_approval": require_approval, "approved": approval_granted},
    )
...
def retraining_flow(
    ...
    summary_markdown = create_retraining_summary(decision, approved, training_results)
...
def main():
    ...
    parser.add_argument(
        "--reference", type=str, required=True, help="Reference data for monitoring"
    )
    parser.add_argument(
        "--current", type=str, required=True, help="Current production data"
    )
    parser.add_argument(
        "--training-data", type=str, required=True, help="Data for retraining"
    )
    parser.add_argument(
        "--current-type",
        type=str,
    ...
    parser.add_argument(
        "--no-approval", action="store_true", help="Skip manual approval requirement"
    )
    parser.add_argument(
        "--drift-threshold", type=float, default=0.5, help="Data drift threshold"
    )
    parser.add_argument(
    ...
    try:
        ...
        if results["training_results"]:
            print(
                f"✅ Retraining completed. Run ID: {results['training_results']['final_run_id']}"
            )
