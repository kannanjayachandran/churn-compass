...
def load_customer_data(source: str, source_type: str = "parquet") -> pd.DataFrame:
    """
    ...
    logger.info(
        "Loading customer data", extra={"source_type": source_type, "source": source}
    )
...
@log_execution_time(logger)
def score_customers(df: pd.DataFrame, model_stage: str = "Production") -> pd.DataFrame:
    """
...
def generate_top_k_list(
    results_df: pd.DataFrame, k_percent: Optional[float] = None
) -> pd.DataFrame:
    """
    ...
    logger.info("Generating top-K list", extra={"k": k, "k_percent": k_percent})
...
def save_results_to_file(
    results_df: pd.DataFrame, output_path: str, format: str = "parquet"
) -> str:
    """
...
def scoring_flow(
    ...
    if output_table:
        rows_written += save_results_to_database(
            results_df, table_name=output_table, if_exists="replace"
        )
...
def main():
    ...
    parser.add_argument("--output-file", type=str, help="Database table for all scores")
    parser.add_argument("--top-k-table", type=str, help="Database table for top-K list")
    parser.add_argument(
        "--top-k-file",
        type=str,
        default=None,
